...
@implementation RSAEncryptorKeyHelper {
...
- (BOOL)verifyData:(NSData *)originalData withSHA1Sign:(NSString *)signString {
    ...
    const char *message = originalData.bytes;
    int messageLength = (int)originalData.length;
...
- (BOOL)verifyData:(NSData *)originalData withMD5Sign:(NSString *)signString
{
    ...
    MD5_CTX ctx;
    MD5_Init(&ctx);
    MD5_Update(&ctx, message, messageLength);
...
- (NSString *)signDataWithSHA1:(NSData *)data
{
    ...
    int messageLength = (int)data.length;
...
- (NSData *)encryptWithPublicKeyForData:(NSData *)originalData {
    if (!_rsa_pub) {
        NSLog(@"please import public key first");
        return nil;
    }
    NSData *contentData = [originalData copy];
    ...
    [contentData getBytes:input length:contentData.length];
...
- (NSString *)encryptWithPublicKeyForString:(NSString *)originalStr {
    NSData *oriData = [originalStr dataUsingEncoding:NSUTF8StringEncoding];
    NSData *encryptedData = [self encryptWithPublicKeyForData:oriData];
...
- (NSData *)decryptWithPrivatecKeyForData:(NSData*)encryptedData {
    ...
    NSData *data = [encryptedData copy];
    ...
    if (status) {
        NSData *returnData = [NSData dataWithBytes:decData length:status];
        free(decData);
        decData = NULL;
...
- (NSString *)decryptWithPrivatecKeyForBase64String:(NSString *)encryptedBase64String {
    ...
    NSData *encryptedData = [[NSData alloc] initWithBase64EncodedString:encryptedBase64String
                                                                options:NSDataBase64DecodingIgnoreUnknownCharacters];
    NSData *decryptedData = [self decryptWithPrivatecKeyForData:encryptedData];
